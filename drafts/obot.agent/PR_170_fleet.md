...
- **CI green: 660 tests, 659 passing, 1 skipped, 0 failing** — [run 32004831192](https://github.com/jwildfire/obot.agent/actions/runs/32004831192), on the rebase onto `121b793`. 61 of those tests are new here. Earlier in this session `tools/ops-dashboard/test/serve-marker.test.mjs` failed locally and it passes now with nothing about it changed: it assumes port 7399 is free, and whether it passes depends on whether another agent happens to be previewing the dashboard. That is the defect rather than a mitigation, it reproduces red on a clean `origin/main` at `7f68cdf`, and the Navigator has filed it as #173. Not this PR's to fix.
