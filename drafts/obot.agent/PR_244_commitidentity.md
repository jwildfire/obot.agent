<!-- STATUS: Posted to https://github.com/jwildfire/obot.agent/pull/244 on 2026-08-18 10:42 EDT -->
...
Tests: 1,143 pass, `python3 scripts/obot-policy validate` clean. The scan test commits through real git rather than a fixture, because the sweep runs against real checkouts from launchd — and it is what proves the property the design rests on: the environment beats the checkout's own config, and beats `-c` on the same command line.

Coordinated with 👯🤖 W0059 (obot.agent#240 and #243, hub#256), which added a `## Local-only work` section to the same file. Its work merged first, so this branch is rebased onto it: the section order is Checkout → Local-only work → Commit identity → RC queue, and `renderState` carries both keys.
