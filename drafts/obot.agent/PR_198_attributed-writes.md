<!-- STATUS: Posted to https://github.com/jwildfire/obot.agent/pull/198 on 2026-08-18 00:52 EDT; body updated 2026-08-20 20:01 EDT -->
...
<b>Tests.</b> 13 new across two files on the original defect, plus 2 on the audit lane; 7 more added here (5 on the credential check, 1 encoding #234's probe table, 1 pinning that a refusal describes a route that exists). Each was confirmed red against this branch before the fix: 5 failures on the guard, 1 on the wrapper. Suite figures re-measured on 2026-08-20 after this branch was brought up to date with <code>main</code>: <b>1705 tests, all passing in CI</b> (<a href="https://github.com/jwildfire/obot.agent/actions/runs/32430926147">run 32430926147</a>, green on the merged head), against 1107 when this paragraph was first written — the growth is main's, not this branch's. The three test files this PR touches carry 33 of them, all green. One test fails <i>locally</i> and only locally (<code>tools/ops-dashboard/test/serve-marker.test.mjs</code>, the second-instance marker case): it fails byte-identically on <code>origin/main</code> whenever this machine's own dashboard holds the port, so it is environmental and pre-existing rather than anything here. <code>obot-policy validate</code> passes, and <code>obot-merge --check</code> reports <code>CHECK PASSED</code> on the attested lane.
...
<b>Brought up to date with <code>main</code> on 2026-08-20, by merge rather than by rebase.</b> This pull request was <code>CONFLICTING</code> for two days and could not have been merged whatever anyone decided. Exactly one file conflicted, <code>AGENTS.md</code>, at exactly one seam: both sides appended a new <code>##</code> section immediately after the merge-invocation section — this branch's "Running a GitHub write" and main's "Branch protections" (<a href="https://github.com/jwildfire/obot.roadmap/issues/272">roadmap#272</a>). Neither edits a line of the other, so both are kept, main's first, with no prose from either side reworded, dropped or reordered. Nothing else in 25 commits of main conflicted. A rebase was rejected on purpose: <code>origin/oa197-attributed-writes</code> was already published, so rebasing would need a force-push, which this workspace forbids and which would have overwritten the 18 August rebase for a second time. <code>hooks/</code> is untouched by the merge — <code>git diff b1ae6d4 -- hooks</code> is empty — so the guard is byte-identical to what was reviewed, and it was driven rather than read off the diff: 17 commands fed through the hook on the merged tree, 9 denied and 8 admitted, every verdict as expected, including a bare <code>gh api -X PATCH</code> DENIED and <code>T=$(mint)</code> + <code>GH_TOKEN=$T</code> ADMITTED.
...
<b>The guard armed in the live workspace is the copy from 17 August, not this branch's.</b> <code>.claude/hooks/attribution-guard.sh</code> is a file copy rather than a link, so both defects above are live in every session running right now, and nothing here changes that until this merges and the hook is re-installed. Re-verified 2026-08-20: the armed copy is still dated 17 August and still differs from this branch's — it admits any <code>GH_TOKEN=</code> prefix, including the substitution form that cannot fail safely, and it carries the #234 segment defect. <code>hooks/install.sh --check</code> from this branch reports it <code>drifted</code>; the same command from <code>main</code> does not mention the guard at all, because main's hook list has three entries and no attribution guard exists there. It was deliberately not updated in place: <code>hooks/</code> is a carve-out path, and arming unapproved code across every session is the thing the carve-out exists to prevent. Sessions started before an install do not have it either — hooks load at session start.
...
<b>No longer fixed here — <code>main</code> got there first.</b> This paragraph used to claim a <code>tool-invocation.test.mjs</code> fix for <code>tools/config-count</code> (commit <code>207f8a5</code>, against merge base <code>17e4f62</code>). Neither is true any more: that commit is on no branch that survives, the six lines are documented by their bare path on <code>main</code> already, and the merge base moved twice. The claim is corrected rather than deleted, because the lesson it carried still stands — <code>config-count</code> landed on <code>main</code> with the suite already red, so the test that exists for this habit was reporting a failure nobody was reading.
...
Drafted by 👯🤖 W0043, extended by 👯🤖 W0049 (rebase), 👯🤖 W0058 and 👯🤖 W0093 (conflict resolution and body re-verification, Claude Code using Opus 5). Awaiting review by @jwildfire — <code>hooks/</code> is a carve-out path, so this merges only with his explicit approval and <code>--jeremy-approved</code>.
