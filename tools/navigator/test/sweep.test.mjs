...
test('EVERY renderState call site passes admiral — including the policy-failure one', () => {
  ...
  // rather than a return value for the same reason.
  const src = readFileSync(join(dirname(fileURLToPath(import.meta.url)), '..', 'sweep.mjs'), 'utf8')
  const calls = src.split('renderState({').slice(1)
