...
//
// Enforced in the tool rather than only in the manager's skill file, because a rule
...
const ws = () => mkdtempSync(join(tmpdir(), 'fleetguard-'))
...
test('the fleet manager is REFUSED a verdict, and told what to do instead', () => {
  const w = ws()
  const r = as('fleet', w, VERDICT)
  assert.notEqual(r.status, 0, 'a fleet verdict must not be recorded')
...
test('the refusal survives a sub-id, so a manager cannot rename its way past it', () => {
  for (const actor of ['fleet', 'fleet-1', 'fleet:abc']) {
...
test('the same actor CAN record a call — it is barred from judging, not from acting', () => {
  const w = ws()
  const r = as('fleet', w, ['call', '--kind', 'session-closed', '--summary', 'closed W0099'])
...
test("a named actor's call is STAMPED in the record he reads, not only in the journal", () => {
  // The requirement's words: the manager's own actions go in the delivery record
  ...
  const w = ws()
  as('fleet', w, ['call', '--kind', 'pr-landed', '--summary', 'landed obot.agent#170'])
  assert.match(record(w), /· call n0001 · fleet · pr-landed · landed obot\.agent#170/)
})
...
test('the audit still reconciles when calls carry an actor stamp', () => {
  ...
  const w = ws()
  as('fleet', w, ['call', '--kind', 'pr-held', '--summary', 'CI red on obot.agent#171'])
  as('session:abc', w, ['call', '--kind', 'requirement-filed', '--summary', 'filed hub#237'])
  const a = as('fleet', w, ['--audit'])
